...
Login2Activity loginActivity;
    ...
    ProgressDialog p;
    public LoginAsyncTask(Login2Activity loginActivity) {
...
//            connection.setRequestProperty("Accept", "application/json");
    ...
    protected void onPostExecute(String s) {
        ...
        try {
            ...
            System.out.println("auth status----"+loginDetailsList.get(0).getAuth_status());

            // Extract values from the first object in the array
            if (loginDetailsList.get(0).getAuth_status().equals("Y")) {
                LoginBean loginDetails = loginDetailsList.get(0); // Get the first object

                // Store values in string variables
                String login_status = loginDetails.getAuth_status();
                String user_nm = loginDetails.getUsr_nm();
                loginActivity.setResulttoUI(login_status,user_nm);
                ...
                Toast.makeText(loginActivity,"UserId and Password doesn't match!",Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
